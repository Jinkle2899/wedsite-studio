...
const DEFAULTS = {
  ...
  date: '2025-12-20', venue: 'Taj Falaknuma Palace, Hyderabad',
  venueAddress: 'Engine Bowli, Falaknuma, Hyderabad',
  hashtag: '#MeeraWedVikram', rsvpDeadline: '2025-11-30',
  story: 'Met at NLSIU Bangalore, debated everything, and fell for each other anyway. He proposed at Falaknuma — the most royal setting for the most important question.',
  primaryColor: '#5c6bc0', accentColor: '#1a237e',
...
function LivePreview({ data }) {
  ...
  return (
    <div className="bg-gradient-to-b from-indigo-50 to-white rounded-2xl overflow-hidden border border-indigo-100 shadow-sm">
      ...
      <div className="px-6 py-10 text-center">
        <div className="w-12 h-12 border-2 rounded-full flex items-center justify-center mx-auto mb-4 bg-white shadow-sm text-xl" style={{ borderColor: data.primaryColor + '60' }}>👑</div>
        <p className="text-xs tracking-[0.3em] uppercase mb-3" style={{ color: data.primaryColor }}>A Royal Celebration</p>
        ...
        <div className="flex justify-center gap-2 mb-6">
          {[['42', 'Days'], ['08', 'Hrs'], ['30', 'Min'], ['00', 'Sec']].map(([n, l]) => (
            <div key={l} className="flex flex-col items-center">
              <div className="w-11 h-11 rounded-xl flex items-center justify-center font-display text-base font-bold text-white shadow-sm"
                style={{ background: data.primaryColor }}>{n}</div>
              <p className="text-xs mt-1" style={{ color: data.primaryColor }}>{l}</p>
            </div>
          ))}
        </div>
        <div className="flex flex-wrap justify-center gap-2 mb-4">
          {['Our Story', 'Schedule', 'RSVP', 'Registry'].map(s => (
            <span key={s} className="text-xs px-3 py-1 rounded-full border-2 font-semibold"
              style={{ borderColor: data.primaryColor, color: data.primaryColor }}>{s}</span>
          ))}
        </div>
        {data.story && <div className="bg-indigo-50 rounded-xl p-3 border border-indigo-100 text-left">
      ...
      <div className="grid grid-cols-3 gap-1 px-4 pb-4">
        {[1,2,3,4,5,6].map(n => {
          const url = data[`photo${n}`] || `https://picsum.photos/seed/rs-g${n}/200/200`
          return <img key={n} src={url} alt={`Photo ${n}`} className="w-full aspect-square object-cover rounded-lg" loading="lazy" />
        })}
      </div>
      <div className="py-3 text-center border-t border-indigo-100" style={{ background: data.accentColor }}>
...
export default function RoyalSapphireEdit() {
    ...
    <div className="min-h-screen bg-gray-50 font-body">
      <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-3">
          <Link to="/templates/royal-sapphire" className="text-sm text-gray-400 hover:text-gray-600">← Preview</Link>
          <span className="text-gray-200">|</span>
          <p className="font-display text-lg font-semibold">Customise: <span className="text-indigo-700">Royal Sapphire</span></p>
        </div>
        <div className="flex items-center gap-3">
          <Link to="/templates/royal-sapphire" className="text-sm text-gray-500 px-4 py-2 border border-gray-200 rounded-full">Preview Live</Link>
          <button onClick={handleSave} className="text-sm font-semibold bg-indigo-700 text-white px-5 py-2 rounded-full hover:bg-indigo-900 transition-colors">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex flex-col gap-6">
          ...
          <div>
            ...
            <div className="flex flex-col gap-4">
              ...
              <Field label="Wedding Hashtag" name="hashtag" value={data.hashtag} onChange={handle} hint="e.g. #MeeraWedVikram" />
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[['Primary (Sapphire)', 'primaryColor'], ['Accent (Navy)', 'accentColor']].map(([label, name]) => (
                <div key={name}>
                  <label className="block text-xs font-semibold text-gray-600 mb-1">{label}</label>
          ...
          <button onClick={handleSave} className="w-full py-3 bg-indigo-700 text-white font-semibold rounded-xl hover:bg-indigo-900 transition-colors text-sm">
